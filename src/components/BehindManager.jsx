...
const BehindManager = () => {
    ...
    const load = async () => {
        ...
        setItems(bts || []); setStories(st || []); setLoading(false);
    };
    ...
    const openEdit = (item) => {
        setEditItem(item);
        setForm({ story_id: item.story_id || '', story_title: item.story_title || '',
            tagline: item.tagline || '', content: item.content || '', written_at: item.written_at || '' });
        setDialogOpen(true);
    };
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        const payload = { story_id: form.story_id, story_title: form.story_title,
            tagline: form.tagline || null, content: form.content, written_at: form.written_at || null };
    ...
    return (
        <Box sx={{ minHeight: '100vh', bgcolor: '#f5ede3', pt: '64px', '&::-webkit-scrollbar': { display: 'none' }, scrollbarWidth: 'none' }}>
            <Container maxWidth="sm" sx={{ py: 5 }}>

                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 4 }} dir="rtl">
                    <Button variant="contained" startIcon={<AddIcon />} onClick={openAdd}
                        sx={{ background: 'linear-gradient(135deg,#c8860a,#e8a830)', color: '#fff', fontWeight: 700,
                            boxShadow: 'none', '&:hover': { boxShadow: '0 4px 14px rgba(200,134,10,0.35)' }, borderRadius: 2, px: 2.5 }}>
                        הוסף
                    </Button>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        <Typography variant="h5" sx={{ fontWeight: 800, color: '#3b2008' }}>ניהול מאחורי הקלעים</Typography>
                        <TheaterComedyIcon sx={{ fontSize: 32, color: '#c8860a' }} />
                    </Box>
                </Box>
                ...
                {message.text && (
                    <Box sx={{
                        ...
                        bgcolor: message.type === 'error' ? 'rgba(211,47,47,0.08)' : 'rgba(46,125,50,0.08)',
                ...
                {loading ? (
                    ...
                    <Typography sx={{ color: '#7a5c3a', textAlign: 'center', mt: 6 }}>אין פריטים עדיין</Typography>
                ) : (
                    items.map(item => (
                        <Box key={item.id} dir="rtl" sx={{
                            mb: 2, p: 2.5, borderRadius: 3, bgcolor: '#fffaf5',
                            border: '1px solid rgba(200,134,10,0.12)',
                            transition: 'border-color 0.2s, box-shadow 0.2s',
                            '&:hover': { borderColor: '#c8860a', boxShadow: '0 0 12px rgba(200,134,10,0.15)' },
                        }}>
                            <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1.5 }}>
                                ...
                                <Box sx={{ flex: 1, minWidth: 0 }}>
                                    <Typography sx={{ fontWeight: 700, color: '#3b2008', mb: 0.5 }} style={rtlStyle}>
                                        {item.story_title || '—'}
                                    </Typography>
                                    {item.tagline && (
                                        <Typography variant="body2" sx={{ color: '#a0622a', fontStyle: 'italic', mb: 0.5 }} style={rtlStyle}>
                                            {item.tagline}
                                        </Typography>
                                    )}
                                    <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mt: 0.5 }}>
                                        {item.written_at && (
                                            <Chip label={new Date(item.written_at).toLocaleDateString('he-IL')} size="small"
                                                sx={{ bgcolor: 'rgba(200,134,10,0.1)', color: '#7a5c3a', fontSize: 11 }} />
                                        )}
                                        <Chip label={`${item.content?.length || 0} תווים`} size="small"
                                            sx={{ bgcolor: 'rgba(200,134,10,0.1)', color: '#7a5c3a', fontSize: 11 }} />
                                    </Box>
                                </Box>
                                <Box sx={{ display: 'flex', gap: 0.5, flexShrink: 0 }}>
                                    <Tooltip title="צפה בדף">
                                        <IconButton size="small" component={Link} to={`/behind/${item.story_id}`} state={{ from: '/manage-behind' }}
                                            sx={{ color: '#c8860a', bgcolor: 'rgba(200,134,10,0.08)', '&:hover': { bgcolor: 'rgba(200,134,10,0.18)' } }}>
            ...
            <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullWidth maxWidth="sm" dir="rtl"
                ...
                <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, pt: '24px !important' }}>
                    <TextField select label="סיפור *" fullWidth value={form.story_id} onChange={handleStorySelect}>
                        {stories.map(s => <MenuItem key={s.id} value={s.id}>{s.title}</MenuItem>)}
                    </TextField>
                    <TextField label="ציטוט פותח (אופציונלי)" fullWidth value={form.tagline}
                        onChange={e => setForm(p => ({ ...p, tagline: e.target.value }))}
                        placeholder="משפט קצר ומסקרן שיופיע בהדגשה..."
                        inputProps={{ style: rtlStyle }} />
                    <TextField label="תוכן *" fullWidth multiline rows={8} value={form.content}
                        onChange={e => setForm(p => ({ ...p, content: e.target.value }))}
                        placeholder="ספרי את הסיפור שמאחורי הסיפור..."
                        inputProps={{ style: { ...rtlStyle, lineHeight: 2 } }} />
                    <TextField label="תאריך כתיבה (אופציונלי)" type="date" fullWidth value={form.written_at}
                        onChange={e => setForm(p => ({ ...p, written_at: e.target.value }))}
                        InputLabelProps={{ shrink: true }} />
                </DialogContent>
                <DialogActions sx={{ p: 2.5, gap: 1, borderTop: '1px solid rgba(200,134,10,0.15)' }}>
                    <Button onClick={() => setDialogOpen(false)} sx={{ color: '#7a5c3a' }}>ביטול</Button>
                    <Button onClick={handleSave} disabled={saving} variant="contained"
                        sx={{ background: 'linear-gradient(135deg,#c8860a,#e8a830)', color: '#fff', fontWeight: 700,
                            boxShadow: 'none', '&:hover': { boxShadow: '0 4px 14px rgba(200,134,10,0.35)' }, px: 3 }}>
                        {saving ? <CircularProgress size={20} sx={{ color: '#fff' }} /> : 'שמור'}
                    </Button>
                </DialogActions>
            </Dialog>
            ...
            <Dialog open={!!deleteId} onClose={() => setDeleteId(null)} dir="rtl"
                PaperProps={{ sx: { bgcolor: '#fffaf5', borderRadius: 3, border: '1px solid rgba(200,134,10,0.15)' } }}>
                <DialogTitle sx={{ color: '#c62828' }}>מחיקת פריט</DialogTitle>
                <DialogContent>
                    <Typography sx={{ color: '#3b2008' }} style={rtlStyle}>האם למחוק את הפריט הזה לצמיתות?</Typography>
                </DialogContent>
                <DialogActions sx={{ p: 2 }}>
                    <Button onClick={() => setDeleteId(null)} sx={{ color: '#7a5c3a' }}>ביטול</Button>
